...
import {
  serviceGetDetail,
  serviceGetTableList, servicePostCreate, servicePostDel, servicePostLogin, servicePostRegister,
} from './services';
...
const model: TModel = {
  ...
  actions: (_params) => {
    ...
    return {
      ...
      postPostRegister(params: any) {
        return servicePostRegister(params).then((d: any) => {
          (basic as any).updateUserInfo(d);
        });
      }
    };
