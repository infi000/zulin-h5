...
const fieldRules = (rule, label) => {
  ...
  const rules = {
    ...
    name: () => ({
      pattern: /^[A-Za-z\u4E00-\u9FA5\uf900-\ufa2d·s]{1,15}$/,
      message: '仅限汉字和英文且不超过15位!',
    }),
  };
  return rules[rule];
};
...
Form.renderNodes = (form, data, disabledFields, formColon) => (nodes) => {
  ...
  return React.Children.map(nodes, (node) => {
    ...
    // 是否为嵌套的表单域（`a` and `a.b`）
    const isNested = nodeProps.form !== undefined;
    ...
    return (
      ...
      >
        {Form.createItems(before)}
        {form.getFieldDecorator(isNested ? `${id}.nested` : id, {
 ...
 * @param {Array} [keys] - 当数据源条目中的 key 不是 `id` (值) 与 `label` (展示的汉字) 时传入
...
class Select extends Ant.Select {
  ...
  render() {
    ...
    const [value = 'id', label = 'label'] = keys;
